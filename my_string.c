  ...
  while(n<size1 && n<size2)
    {
      if( (int) *pString1->at((MY_STRING)pString1, n) < (int) *pString2->at((MY_STRING)pString2, n))
        return -1;
      else if( (int) *pString1->at((MY_STRING)pString1, n) > (int) *pString2->at((MY_STRING)pString2, n))
        return 1;
      n++;
    }
  if(size1<size2)
...
Status my_string_concatenation(MY_STRING hMy_string_dest, MY_STRING hMy_string_addition)
{
  ...
  size2 = dest->get_size((MY_STRING) addition);
  if(addition->get_size((MY_STRING) addition) == 0)
        return FAILURE;
  for(i=0; i<size2; i++)
    {
